...
function UsableSkill:getMaxUseTime(player, scope, card, to)
  ...
  for _, skill in ipairs(Fk:currentRoom().status_skills[TargetModSkill] or Util.DummyTable) do
    ret = ret + (skill:getResidueNum(player, self, scope, card, to) or 0)
  end
  return ret
end
...
function UsableSkill:withinTimesLimit(player, scope, card, card_name, to)
  ...
  return player:usedCardTimes(card_name, scope) < self:getMaxUseTime(player, scope, card, to)
      or (player:getMark(MarkEnum.BypassTimesLimit) ~= 0
        or table.find(temp_suf, function(s)
          return player:getMark(MarkEnum.BypassTimesLimit .. s) ~= 0
        end))
      or (to:getMark(MarkEnum.BypassTimesLimitTo) ~= 0
        or table.find(temp_suf, function(s)
...
function UsableSkill:withinDistanceLimit(player, isattack, card, to)
  ...
  local temp_suf = table.simpleClone(MarkEnum.TempMarkSuffix)
  table.insert(temp_suf, "-tmp")
  return (isattack and player:inMyAttackRange(to))
      or (player ~= to and player:distanceTo(to) <= self:getDistanceLimit(player, card, to))
      or (player:getMark(MarkEnum.BypassDistancesLimit) ~= 0
        or table.find(temp_suf, function(s)
          return player:getMark(MarkEnum.BypassDistancesLimit .. s) ~= 0
        end))
      or (to:getMark(MarkEnum.BypassDistancesLimitTo) ~= 0
        or table.find(temp_suf, function(s)
